...
### FILE CONTAINING DATA AND FUNCTIONS ASSOCIATED WITH THE MINIHACK ENVS I AM GONNA BE USING ###


#These files first of all defines the ENVIRONMENT IDs -> flags that we use to create different environments.
#"empty-room", "room-with-lava", "room-with-monster", "room-with-multiple-monsters", "cliff-minihack"
#
#empty-room and room_with_multiple_monsters are standard MiniHack environments
#"room-with-lava", "room-with-monster", "cliff-minihack" are custom minihack environments (defined through gym) we define with the help of des_file.
#The des_file is ASCII code to define the layout and behaviour of the room
#
#to create environment i call get_minihack_environment with syntax (id, size = 5, random = False, add_pixels = False, max_episode_steps = 1000)
#size -> size x size map, random -> if the layout and the monsters are random, max_episode_steps -> We say that the game is TRUNCATED when it finished by max steps, TERMINATED when it reaches terminal state.
#
#In minihack (and Nethack Learning Environment in general) the obs is returned as char array representation. 
#I can also get observation in form of rendered image or other forms .
#I can use the rendered image representation to see visually the information that is represented "encrypted" inside the char representation (just imshow)
#This is the "pixel" output option. 
#
# 32  -> '', Blank 
# 35  -> '#',  Tree
# 45  -> '-',  Horizontal Wall
...
class DoNotResetWhenDead(gym.Wrapper):
    ...
    def step(self, action, **kwargs):
        ...
        if info["end_status"] == env.StepStatus.ABORTED or self.step_counter == self.max_episode_steps:
            if info["end_status"] == env.StepStatus.ABORTED:
                print("THe game just got aborted on step", self.step_counter)
            terminated = truncated = True
            reward = self.negative_step_reward
        elif info["end_status"] == env.StepStatus.DEATH:
            # print("The agent dies!")
